...
document.addEventListener('click', ({target}) => {
  if (target.dataset.cmd === 'download') {
    const next = () => {
      ...
      req.onerror = () => {
        toast.notify('Something went wrong! Please download the package manually', 'error', 3000);
        window.setTimeout(() => {
    ...
    if (chrome.downloads) {
      ...
      chrome.permissions.request({
        ...
        if (granted) {
          ...
          toast.notify('Cannot initiate file downloading. Please download the file manually', 'error', 60000);
        }
      });
    }
  }
